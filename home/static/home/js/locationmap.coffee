class LocationMap extends google.maps.Map
    constructor: (mapDiv) ->
        @focusName     = "Mt. Albert Methodist Church"
        @mapCentre     = lat: -36.877000, lng: 174.719608
        @focusLocation = lat: -36.881947, lng: 174.719608
        @focusAddress  = "831 New North Road, Auckland"
        if $(document).width() > 480
            # If document (your website) is wider than 480px
            @isDraggable = true
        else
            @isDraggable = false
...
$ ->
    ...
    if locationMap.length
        # azteca gives v3.21 controls in the v3.22 API
        # google.maps.controlStyle = 'azteca'
        new LocationMap(locationMap[0])
    return

$(document).on 'touchstart', 'div[title^=Pan]', () ->
    $(this).trigger('click')
    return false
